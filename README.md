...
The pipeline takes S.pneumoniae reads and uses SeroBA to determine their serotype. It then assembles the reads using Unicycler in bold mode (to avoid contig breaks where possible).
...
If any gaps are determined, these are filled in using a consensus sequence method. The CPS sequence is annotated using Bakta and checked for any potentially disruptive mutations using ARIBA.
...
acid sequences for each gene will be concatenated for easy alignment and tree building. Additionally, a gene comparison plot will be generated for samples containing disruptive mutations using clinker and genetic variants will be shown using clinker.
## Output
